...
export class DefaultDataForNewUser {
  static create (user: UserDBO) {
    ...
    TagModel
      .insertMany(DefaultDataForNewUser.getTags(userId, tagIdStartup, tagIdProductivity, tagIdVideo))
      .then(() => {
        ItemModel
          .insertMany(DefaultDataForNewUser.getItems(userId, tagIdStartup, tagIdProductivity, tagIdVideo))
          .catch(error => {
